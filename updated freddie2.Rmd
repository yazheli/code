...
output: pdf_document
---
$min\frac{1}{2N}\sum _{i=1}^{N} {(y_i-\beta_0-\beta x_i ^ \mathrm{ T } ) ^2+\lambda[(1-\alpha)*\left \|  \beta\right \|_2^2/2+\alpha*\left \|  \beta\right \|_1]}$
...
```
```{r}
default_group = numerical[which(numerical$def_flag==TRUE),]
no_default_group = numerical[which(numerical$def_flag==FALSE),]
default_group = default_group[,-c(7,9)]
no_default_group = no_default_group[,-c(7,9)]

ggpairs(no_default_group[1:2000,],lower = list(continuous = wrap("points", alpha = 0.3,    size=0.1)),title = "Nodefault Scatterplot Matrix",axisLabels = "none")

ggpairs(default_group[1:2000,],lower = list(continuous = wrap("points", alpha = 0.3,    size=0.1)),title = "Default Scatterplot Matrix",axisLabels = "none")
```
...
```{r, warning=FALSE}
temp = as.data.frame(cbind(unlist(auc_matrix["GA",]),rownames(auc_matrix)))
...
state_choropleth(temp, title = "AUC on Different States based on Model from Georgia",num_colors = 9,legend = "AUC")
```
