...
library(parallel)
results = mclapply( seq(20, 50,by=10), FUN=ten_time_auc )
save(results,file="un_sample_rf.RData")

auc = as.data.frame(c(results[[1]],results[[2]]))
auc$per = c(rep(40,5),rep(50,5))
names(auc)=c('auc','per')
tg <- summarySE(auc, measurevar="auc", groupvars='per')
